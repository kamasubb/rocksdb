...
/* This wrapper function works around the fact that ceph_crc32c_ppc_internal
...
uint32_t crc32c_ppc(uint32_t crc, unsigned char const *data, unsigned len)
{
	...
	if (!data) {
		...
		bzero(buf2, len);
		crc = ceph_crc32c_ppc_internal(crc, buf2, len);
		free(buf2);
	} else {
		crc = ceph_crc32c_ppc_internal(crc, data, (unsigned long)len);
	}
	return crc;
}
